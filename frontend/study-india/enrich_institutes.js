...
// We need to extract the institutes array, modify it, and put it back.
// The array starts with `export const studyIndiaInstitutes: OfficialUniversity[] = [`

const institutesStartIndex = content.indexOf('export const studyIndiaInstitutes: OfficialUniversity[] = [');
if (institutesStartIndex === -1) {
  console.error('Could not find institutes array');
  process.exit(1);
}

const coursesStartIndex = content.indexOf('export const studyIndiaCourses: OfficialCourse[] = [');
if (coursesStartIndex === -1) {
  console.error('Could not find courses array');
  process.exit(1);
}

// Extract just the institutes part
...
// We'll use a regex to replace fields in each object
// Since parsing a massive TS file with regex can be tricky, let's replace NIRF and Description for each institute
...
const topRanks = {
  ...
  "Indian Institute Of Management Ahmedabad": 1, // Management
  ...
  "Hindu College Delhi": 1, // College
  "Miranda House": 2, // College
};

function getDeterministicRank(name) {
  // If it's a known top institute, return its rank
  ...
  // Otherwise generate a deterministic realistic rank based on string hash
  let hash = 0;
...
// We will use regex to find each object and replace fields
