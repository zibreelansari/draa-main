...
export default function StudentDashboard({ data, activeSection, onNavigate, onRefresh }: DashboardViewProps<StudentWorkspace>) {
  ...
  if (activeSection === "documents") return <>
    ...
    <section className="workspace-panel" data-dashboard-reveal><SectionHeader title="Your checklist" eyebrow="Required records" />
      <div className="document-list">{data.documents.map((document) => <article key={document.id}><span className={`document-icon ${document.status.toLowerCase().replace(/_/g, "-")}`}><FileText size={20} /></span><div><h3>{document.documentType}</h3><p>{document.fileName || "No file supplied"}</p></div><StatusBadge value={document.status} /><small>Updated {formatDate(document.updatedAt)}</small></article>)}</div>
    </section>
