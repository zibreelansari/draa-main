...
export default function GlobalMinimalMotionBackground() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      time += 0.001; // Ultra-slow, serene, non-distracting pace
      ctx.clearRect(0, 0, width, height);

      // Draw faint, elegant harmonic filament waves across the viewport
      const waveCount = 3;
      ...
      // Draw connective lines between nearby drifting particles
      ...
      // Render individual drifting particles
      particles.forEach((p) => {
        p.x += p.vx;
        p.y += p.vy;
        ...
        p.pulse += 0.008;
        const opacity = (0.2 + Math.sin(p.pulse) * 0.15).toFixed(3);

        ctx.beginPath();
        ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2);
        ctx.fillStyle = p.isGold
          ? `rgba(189, 123, 32, ${opacity})`
          : `rgba(217, 164, 70, ${opacity})`;
        ctx.fill();

        // Subtle outer glow
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.radius * 2.2, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(245, 158, 11, ${(Number(opacity) * 0.2).toFixed(3)})`;
        ctx.fill();
      });
