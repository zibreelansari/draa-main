...
type FormStatus = { type: 'idle' | 'success' | 'error'; message?: string; reference?: string };
...
export default function DraaCorporateContact() {
  ...
  const submitEnquiry = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus({
        type: 'success',
        message: 'Thank you. Our team has received your enquiry and will contact you shortly.',
            ...
            <div className="draa-contact-form-card">
              ...
              {status.type === 'success' ? (
                <div className="draa-contact-success" role="status">
                  ...
                  <p>{status.message}</p>
                  {status.reference && <span>Reference: {status.reference}</span>}
                ...
                <form onSubmit={submitEnquiry} className="draa-contact-form">
                  ...
                  {status.type === 'error' && <p className="draa-contact-error" role="alert">{status.message}</p>}
                  <button type="submit" className="draa-contact-submit" disabled={submitting}>
                    {submitting ? 'Sending enquiry…' : <>Send enquiry <Send size={17} /></>}
                  </button>
                </form>
              )}
            </div>
